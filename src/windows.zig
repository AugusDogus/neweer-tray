...
fn toLowerAscii(c: u16) u8 {
    if (c >= 'A' and c <= 'Z') return @truncate(c + ('a' - 'A'));
    if (c < 128) return @truncate(c);
    return 0;
}
...
fn pathContainsVidPid(path: [*:0]const WCHAR) bool {
    ...
    if (i < VID_PID_PATTERN.len) return false;

    var j: usize = 0;
    while (j <= i - VID_PID_PATTERN.len) : (j += 1) {
        if (std.mem.eql(u8, ascii_buf[j .. j + VID_PID_PATTERN.len], VID_PID_PATTERN)) {
            return true;
        }
    }
    return false;
...
fn findNeewerDongle() bool {
    ...
    while (true) : (device_index += 1) {
        ...
        if (SetupDiEnumDeviceInterfaces(dev_info, null, &GUID_DEVINTERFACE_HID, device_index, &interface_data) == 0) break;
        ...
        if (SetupDiGetDeviceInterfaceDetailW(dev_info, &interface_data, @ptrCast(&detail_buffer), required_size, null, null) == 0) continue;
...
fn openDevice() ?HANDLE {
    if (!g_device_path_valid and !findNeewerDongle()) return null;
    ...
    if (handle == INVALID_HANDLE_VALUE) {
        ...
        if (!findNeewerDongle()) return null;
        return CreateFileW(
            @ptrCast(&g_device_path),
            GENERIC_READ | GENERIC_WRITE,
            FILE_SHARE_READ | FILE_SHARE_WRITE,
...
fn toggleLights() bool {
    ...
    if (handle == null or handle == INVALID_HANDLE_VALUE) return false;
    defer _ = CloseHandle(handle.?);

    const packet = neewer.buildPacket();
...
fn trayAdd(hwnd: HWND) void {
    ...
    if (g_nid.hIcon == null) g_nid.hIcon = LoadIconW(null, IDI_APPLICATION);
